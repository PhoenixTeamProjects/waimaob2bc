...
- 建立项目案例第一阶段框架：独立 `projects` 数据源、本地项目适配层、双语项目案例入口页、真实素材空状态、项目录入标准和 CollectionPage JSON-LD。
- 建立课程培训第一阶段框架：独立 `courses` 数据源、本地课程适配层、双语课程培训入口页、真实资料空状态、课程咨询信息清单和 CollectionPage JSON-LD。
- 建立联系与咨询第一阶段框架：独立咨询类型和字段配置、双语联系页、静态表单占位、隐私边界提示和 ContactPage JSON-LD。
